...
if __name__ == '__main__':
    ...
    if pic_path[-4:] == '.png':  # .png图片在这里的存储格式是0到1的浮点数，所以要扩展到255再计算
        img = img * 255  # 还是浮点数类型
    img = img.mean(axis=-1)  # 取均值就是灰度化了    为什么取均值就是灰度化？
    ...
    tmp = [i - dim // 2 for i in
           range(dim)]  # 生成一个序列 （例如这里是-1 0 1 2 3）     这里面i-dim//2是一个数，其中//是整数除法，能返回商的整数部分，向下取整。后面就是一个for循环
    n1 = 1 / (2 * math.pi * sigma ** 2)  # 计算高斯核
    n2 = -1 / (2 * sigma ** 2)
    for i in range(dim):
        for j in range(dim):
            Gaussian_filter[i, j] = n1 * math.exp(n2 * (tmp[i] ** 2 + tmp[j] ** 2))  # 高斯分布的公式
    Gaussian_filter = Gaussian_filter / Gaussian_filter.sum()
    dx, dy = img.shape
    img_new = np.zeros(img.shape)  # 存储平滑之后的图像，zeros函数得到的是浮点型数据
    tmp = dim // 2
    img_pad = np.pad(img, ((tmp, tmp), (tmp, tmp)),
                     'constant')  # 边缘填补，在图像边缘填补0，使得卷积前后图像大小不变，且不会丢失边缘信息。img的第一个轴两边缘分别添加tmp个0，第二个轴也是。为什么填充的是0？因为填充方式constant缺省（默认）值为0.
    for i in range(dx):
        for j in range(dy):
            img_new[i, j] = np.sum(img_pad[i:i + dim, j:j + dim] * Gaussian_filter)  # 这是怎么一个过程
    plt.figure(1)  # 图片编号1
    ...
    for i in range(1, dx - 1):
        for j in range(1, dy - 1):
            ...
            if angle[i, j] <= -1:  # 使用线性插值法判断抑制与否，需要列式进行计算
    ...
    # 判断强边缘周围是否存在弱边缘，强边缘旁边的弱边缘，也置为强边缘（与PPT中的方法相反，PPT里是看弱边缘周围是否是强边缘）
    while not len(zhan) == 0:
        temp_1, temp_2 = zhan.pop()  # 出栈，栈内不等于0的点，返回其中的内容[i,j]
        a = img_yizhi[temp_1 - 1:temp_1 + 2, temp_2 - 1:temp_2 + 2]  # 取出[i,j]周围的八个点
