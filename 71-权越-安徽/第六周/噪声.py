...
if  __name__=="__main__":
   ...
   noise_gs_img=util.random_noise(source,mode='poisson')
   cv2.imshow("source",source)

   cv2.imshow("poisson",noise_gs_img)
   noise_gs_img = util.random_noise(source, mode='gaussian')
   cv2.imshow("gaussian",noise_gs_img)

   noise_gs_img = util.random_noise(source, mode='localvar')
   cv2.imshow("localvar", noise_gs_img)

   noise_gs_img = util.random_noise(source, mode='salt')
   cv2.imshow("salt", noise_gs_img)
   noise_gs_img = util.random_noise(source, mode='pepper')
   cv2.imshow("pepper", noise_gs_img)

   noise_gs_img = util.random_noise(source, mode='s&p')
   cv2.imshow("s&p", noise_gs_img)

   noise_gs_img = util.random_noise(source, mode='speckle')
   cv2.imshow("speckle", noise_gs_img)
