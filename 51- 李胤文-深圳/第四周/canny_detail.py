...
if __name__ == '__main__':
    ...
    # plt.figure(1)
    plt.subplot(231)
    plt.imshow(img.astype(np.uint8))  # 此时的img_new是255的浮点型数据，强制类型转换才可以，gray灰阶
    plt.axis('off')
    ...
    while not len(zhan) == 0:
        temp_i, temp_j = zhan.pop()  # 出栈,记录的yizhi中255点的i，j
        a = img_yizhi[temp_i-1:temp_i+2, temp_j-1:temp_j+2] # 255点的临接矩阵8个点
        if (a[0, 0] < high_boundary) and (a[0, 0] > lower_boundary):
            img_yizhi[temp_i-1, temp_j-1] = 255  # 这个像素点标记为边缘
            zhan.append([temp_i-1, temp_j-1])  # 进栈
        if (a[0, 1] < high_boundary) and (a[0, 1] > lower_boundary):
            img_yizhi[temp_i - 1, temp_j] = 255
            zhan.append([temp_i - 1, temp_j])
        if (a[0, 2] < high_boundary) and (a[0, 2] > lower_boundary):
            img_yizhi[temp_i - 1, temp_j + 1] = 255
            zhan.append([temp_i - 1, temp_j + 1])
        if (a[1, 0] < high_boundary) and (a[1, 0] > lower_boundary):
            img_yizhi[temp_i, temp_j - 1] = 255
            zhan.append([temp_i, temp_j - 1])
